...
29. `[x]` **Sistema plugin per tab custom** — Permettere a repo esterni di sviluppare nuove tab (es. YoutubeDownloader) caricate a runtime dall'host, senza toccare il codice core. Contract minimo `ITabPlugin` (Id/Header/IconGlyph/CreateView/OnUnload), `PluginLoader` con isolamento `AssemblyLoadContext` (+ `AssemblyDependencyResolver` per dipendenze private del plugin), discovery deterministica e dedup by Id da `~/.config/Sbroglione/plugins` (via `Environment.SpecialFolder.ApplicationData`, skip su Android), tab appese a runtime in `MainView` (`TabControl` data-driven, per-plugin try/catch), `OnUnload()` invocato su ciascun plugin su ogni percorso di shutdown (incl. `ProcessExit`). Guardia path-traversal su `MainAssemblyFileName`. Doc per plugin author in `docs/plugins.md`. Verifica manuale end-to-end eseguita: plugin di prova costruito e installato, tab compare con icona/testo corretti, contenuto funzionante, `OnUnload` verificato scrivendo su file alla chiusura (via `WM_DELETE_WINDOW`); casi di errore verificati senza crash (contractVersion incompatibile → tab assente; `plugin.json` mancante → tab assente). Scope desktop-only (non presente nel ramo Android). *(A)*

30. `[x]` **Spezzare service monolitici in responsabilità più piccole** — `FileSystemService` (e probabilmente altri: `DirectoryComparisonService`, `WatchFolderService`) accumulano più responsabilità distinte in un solo file statico (es. `FileSystemService` fa: path-type query, listing semplice/ricorsivo, create/rename/delete, gestione path UNC, mapping eccezioni→`ListingError`). Valutare split in classi/file dedicati per singola responsabilità, mantenendo la stessa API pubblica dove possibile per non rompere i chiamanti. *(M)* — Fatto: `FileSystemService` diventato facciata pubblica su `FileSystemErrors`/`FileSystemPathQueries`/`FileSystemListingQueries`/`FileSystemMutations`; `WatchFolderService` diventato registro su `RuleRunner` (watcher/loop) + `WatchFolderSyncEngine` (sync). API pubblica invariata. `DirectoryComparisonService` lasciato intatto (algoritmo unico e coeso, non ha il problema). 629/629 test passano.
